...
function Card({ className, ...props }: React.ComponentProps<'div'>) {
	return (
		<div
			className={cn(
				'bg-card relative w-full max-w-xs mx-auto rounded-xl dark:bg-transparent',
				...
				'sm:max-w-sm md:max-w-xs lg:max-w-sm',
				className,
			)}
			{...props}
		/>
...
function Header({
	...
	return (
		<div
			className={cn(
				'bg-black/20 dark:bg-black/30 backdrop-blur-md relative mb-4 rounded-xl border border-white/20 p-4',
				'shadow-lg',
				className,
			)}
			{...props}
		>
			...
			{glassEffect && (
				<div
					...
					style={{
						background:
							'linear-gradient(180deg, rgba(255,255,255,0.1) 0%, rgba(255,255,255,0.05) 40%, rgba(0,0,0,0) 100%)',
					}}
				/>
			)}
			{children}
...
function Description({ className, ...props }: React.ComponentProps<'p'>) {
	return (
		<p className={cn('text-muted-foreground text-xs sm:text-sm dot-background-text', className)} {...props} />
	);
}
...
function PlanName({ className, ...props }: React.ComponentProps<'div'>) {
	return (
		<div
			className={cn(
				"text-muted-foreground flex items-center gap-2 text-sm sm:text-base font-medium [&_svg:not([class*='size-'])]:size-4 dot-background-text",
				className,
			)}
			{...props}
		/>
...
function MainPrice({ className, ...props }: React.ComponentProps<'span'>) {
	return (
		<span
			className={cn('text-2xl sm:text-3xl font-extrabold tracking-tight dot-background-heading', className)}
			{...props}
		/>
	);
}
...
function Body({ className, ...props }: React.ComponentProps<'div'>) {
	return <div className={cn('space-y-4 sm:space-y-6 p-3 sm:p-4', className)} {...props} />;
}
...
function ListItem({ className, ...props }: React.ComponentProps<'li'>) {
	return (
		<li
			className={cn(
				'text-muted-foreground flex items-start gap-3 text-sm sm:text-base dot-background-text leading-relaxed',
				className,
			)}
			{...props}
		/>
